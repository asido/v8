...
void v8_init() {
  std::unique_ptr<v8::Platform> platform_sp = v8::platform::NewDefaultPlatform(
      0, // thread_pool_size
...
ValueTuple v8_Context_Run(ContextPtr ctxptr, const char* code, const char* filename) {
  ...
  if (result.IsEmpty()) {
    ...
    res.Value = static_cast<PersistentValuePtr>(new Value(isolate, val));
    res.Kinds = v8_Value_KindsFromLocal(val);
  }
...
void go_callback(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  if (result.error_msg.ptr != nullptr) {
    ...
    //v8::ReturnValue<Value> return_val = args.GetReturnValue();
